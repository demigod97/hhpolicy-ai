...
CREATE OR REPLACE FUNCTION public.can_access_pdf_storage(
  bucket_name TEXT,
  file_path TEXT,
  user_id UUID
)
...
BEGIN
  ...
  FROM public.sources
  WHERE (pdf_file_path = file_path OR public.sources.file_path = public.can_access_pdf_storage.file_path)
...
CREATE POLICY "role_based_read_access" ON storage.objects
  ...
  USING (
    bucket_id = 'sources'
    AND public.can_access_pdf_storage(bucket_id, name, (SELECT auth.uid()))
  );
